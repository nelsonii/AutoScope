...
// On my AmScope the X knob (smaller) is 26mm
GearHeight=7; //mm -- to fit in the motor housing; 5 gives lots of space
GearAHub=5.25; // mm -- added 0.25 to compensate for my printer
GearBHub=24.25; //mm -- added 1.25 to compensate for my printer
...
union(){
  if (GearAExists){
  cylinder(h=GearHeight,d=GearAHub,center=true);
  }
  if (GearBExists){
  translate([OurCenterDistanceNamedBob,0,0]){
  cylinder(h=GearHeight,d=GearBHub,center=true);
  }
}
}
}
